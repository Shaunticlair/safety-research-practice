...
`starter.py` supplies local Qwen inference, exact grading, raw-output inspection, and paired analysis. The baseline configuration pins the model revision. The cached baseline includes raw outputs, metadata, and the exact runner source. Generator, grading, formatting, and actual GPU runs have been checked; a fresh participant timing trial is still pending.
